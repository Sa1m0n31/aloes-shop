...
con.connect(err => {
    ...
    router.post("/add", (request, response) => {
        ...
        if(name === "") {
            response.redirect("https://aloes.skylo-test3.pl/panel/kategorie?added=0");
            return 0;
        }
        ...
        con.query(query, values, (err, res) => {
            console.log(err);
            if(!err) response.redirect("https://aloes.skylo-test3.pl/panel/kategorie?added=1");
            else response.redirect("https://aloes.skylo-test3.pl/panel/kategorie?added=-1")
        });
    });
    ...
    con.query(query, values, (err, res) => {
        if(!err) response.redirect("https://aloes.skylo-test3.pl/panel/kategorie?added=2");
        else response.redirect("https://aloes.skylo-test3.pl/panel/kategorie?added=-1")
    });
    });
